<?php if ($tambah == 1) { ?>
    ...
    <small class="text-secondary"> Blok Counter berjumlah 4 baris, pastikan ketika menambah data berjumlah minimal 4 data
        jika lebih maka berjumlah 8 dst.</small>
    <hr>
<?php } ?>
            ...
            foreach ($list as $data):
                ...
                <tr>
                    ...
                    <td class="p-1 text-center">
                        <?php if (esc($data['ic']) != '') { ?>
                            <a class="text-warning font-13"> <i class="<?= esc($data['ic']) ?> text-secondary"
                                    title="<?= esc($data['ic']) ?>"></i></a>
                        <?php } else { ?>
                            -
                        <?php } ?>
                    </td>
                    ...
                    <td class="text-center p-1">
                        <?php if ($ubah == 1) { ?>
                            <?php if ($data['sts'] == '1') { ?>
                                <button type="button" onclick="toggle('<?= $data['id_counter'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['sts'] ? 'btn-light' : 'btn-success' ?>"
                                    title="<?= $data['sts'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                                        class="fas fa-check-circle text-success"></i>
                                </button>
                            <?php } else { ?>
                                <button type="button" onclick="toggle('<?= $data['id_counter'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['sts'] ? 'btn-info' : 'btn-light' ?>"
                                    title="<?= $data['sts'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                        ...
                        <?php if ($hapus == 1) { ?>
                            <button type="button" title="Hapus Data" class="btn btn-light btn-sm p-1"
                                onclick="hapus('<?= $data['id_counter'] ?>','<?= $data['nm'] ?>')">
