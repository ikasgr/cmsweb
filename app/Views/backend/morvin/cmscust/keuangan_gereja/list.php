...
<div class="row mb-3">
    <div class="col-md-4">
        <form class="formsearch">
            ...
            <div class="input-group">
                <input type="text" class="form-control" id="keyword" name="keyword" placeholder="Cari kode, keterangan...">
    ...
    <div class="col-md-4">
        <form class="formfilter">
            ...
            <div class="row">
                <div class="col-6">
                    <input type="date" class="form-control" id="filterTanggalMulai" name="tanggal_mulai" placeholder="Dari tanggal">
                </div>
                <div class="col-6">
                    <input type="date" class="form-control" id="filterTanggalSelesai" name="tanggal_selesai" placeholder="Sampai tanggal">
                </div>
            </div>
        </form>
    </div>
...
<div class="table-responsive">
    <table class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="tabelKeuangan">
        ...
        <tbody>
            ...
            <?php foreach ($list as $data) : ?>
                <tr>
                    <td>
                        <?php if ($akses == '1' || $akses == '2') { ?>
                            <input type="checkbox" class="form-check-input centangid" name="id_transaksi[]" value="<?= $data['id_transaksi'] ?>">
                        <?php } ?>
                    </td>
                    <td><?= $nomor++ ?></td>
                    ...
                    <td>
                        <div class="btn-group" role="group">
                            <button type="button" class="btn btn-info btn-sm" onclick="lihat('<?= $data['id_transaksi'] ?>')" title="Lihat Detail">
                                <i class="fas fa-eye"></i>
                            </button>
                            
                            <?php if ($akses == '1' || $akses == '2') { ?>
                                <?php if ($data['status'] == 'Pending') { ?>
                                    <button type="button" class="btn btn-warning btn-sm" onclick="approve('<?= $data['id_transaksi'] ?>')" title="Approve">
                                        <i class="fas fa-check-circle"></i>
                                    </button>
                                <?php } ?>
                                
                                <?php if ($data['status'] == 'Pending' || $data['status'] == 'Ditolak') { ?>
                                    <button type="button" class="btn btn-primary btn-sm" onclick="edit('<?= $data['id_transaksi'] ?>')" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                <?php } ?>
                                
                                <button type="button" class="btn btn-danger btn-sm" onclick="hapus('<?= $data['id_transaksi'] ?>', '<?= esc($data['kode_transaksi']) ?>')" title="Hapus">
