    ...
    <div class="col-md-6">
        <form class="formcari">
            ...
            <div class="input-group">
                <input type="text" class="form-control" id="keyword" name="keyword" placeholder="Cari nama atau nomor anggota...">
...
<div class="table-responsive">
    <table class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="tabelJemaat">
        ...
        <tbody>
            ...
            <?php foreach ($list as $data) : ?>
                <tr>
                    <td>
                        <?php if ($akses == '1' || $akses == '2') { ?>
                            <input type="checkbox" class="form-check-input centangid" name="id_jemaat[]" value="<?= $data['id_jemaat'] ?>">
                        <?php } ?>
                    </td>
                    <td><?= $nomor++ ?></td>
                    ...
                    <td>
                        <div class="btn-group" role="group">
                            <button type="button" class="btn btn-info btn-sm" onclick="lihat('<?= $data['id_jemaat'] ?>')" title="Lihat Detail">
                                <i class="fas fa-eye"></i>
                            </button>
                            
                            <?php if ($akses == '1' || $akses == '2') { ?>
                                <button type="button" class="btn btn-warning btn-sm" onclick="edit('<?= $data['id_jemaat'] ?>')" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </button>
                                
                                <button type="button" class="btn btn-secondary btn-sm" onclick="toggle('<?= $data['id_jemaat'] ?>', '<?= esc($data['nama_lengkap']) ?>')" title="Ubah Status">
                                    <i class="fas fa-exchange-alt"></i>
                                </button>
                                
                                <button type="button" class="btn btn-primary btn-sm" onclick="uploadfoto('<?= $data['id_jemaat'] ?>')" title="Upload Foto">
                                    <i class="fas fa-camera"></i>
                                </button>
                                
                                <?php if ($data['foto']) : ?>
                                    <button type="button" class="btn btn-outline-danger btn-sm" onclick="hapusfoto('<?= $data['id_jemaat'] ?>', '<?= esc($data['nama_lengkap']) ?>')" title="Hapus Foto">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                <?php endif; ?>
                                
                                <button type="button" class="btn btn-danger btn-sm" onclick="hapus('<?= $data['id_jemaat'] ?>', '<?= esc($data['nama_lengkap']) ?>')" title="Hapus">
...
<script>
    $(document).ready(function() {
        ...
        $('#tabelJemaat').DataTable({
            "language": {
                "url": "<?= base_url() ?>/public/template/backend/morvin/assets/js/datatables/Indonesian.json"
            },
            "pageLength": 25,
            "responsive": true,
