...
<div class="table-responsive mt-3">
    <table id="listjadwal" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
        ...
        <tbody>
            ...
            ?>
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-info btn-sm" onclick="lihat('<?= $data['id_jadwal'] ?>')"><i class="fas fa-eye"></i></button>
                            <button type="button" class="btn btn-warning btn-sm" onclick="edit('<?= $data['id_jadwal'] ?>')"><i class="fas fa-edit"></i></button>
                            <button type="button" class="btn btn-danger btn-sm" onclick="hapus('<?= $data['id_jadwal'] ?>')"><i class="fas fa-trash"></i></button>
                        </td>
                    </tr>
            <?php endforeach; else : ?>
...
<script>
    ...
    function tambah() {
        $.ajax({type: 'get', url: '<?= base_url('jadwal-pelayanan/formtambah') ?>', dataType: 'json',
            success: function(response) { $('.viewmodal').html(response.data).show(); $('#modaltambah').modal('show'); }
        });
    }

    function lihat(id) {
        $.ajax({type: 'post', url: '<?= base_url('jadwal-pelayanan/formlihat') ?>',
            data: {csrf_tokencmsdatagoe: $('input[name=csrf_tokencmsdatagoe]').val(), id: id}, dataType: 'json',
            success: function(response) { if (response.sukses) { $('.viewmodal').html(response.sukses).show(); $('#modallihat').modal('show'); }}
        });
    }

    function edit(id) {
        $.ajax({type: 'post', url: '<?= base_url('jadwal-pelayanan/formedit') ?>',
            data: {csrf_tokencmsdatagoe: $('input[name=csrf_tokencmsdatagoe]').val(), id: id}, dataType: 'json',
    ...
    function hapus(id) {
        Swal.fire({title: 'Hapus Jadwal?', text: 'Data akan dihapus permanen!', icon: 'warning', showCancelButton: true,
            confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Ya, Hapus!', cancelButtonText: 'Batal'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({type: 'post', url: '<?= base_url('jadwal-pelayanan/hapus') ?>',
                    data: {csrf_tokencmsdatagoe: $('input[name=csrf_tokencmsdatagoe]').val(), id: id}, dataType: 'json',
    ...
    function hapusall() {
        ...
        Swal.fire({title: 'Hapus ' + id.length + ' Jadwal?', text: 'Data akan dihapus permanen!', icon: 'warning', showCancelButton: true,
            confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Ya, Hapus!', cancelButtonText: 'Batal'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({type: 'post', url: '<?= base_url('jadwal-pelayanan/hapusall') ?>',
                    data: {csrf_tokencmsdatagoe: $('input[name=csrf_tokencmsdatagoe]').val(), id: id}, dataType: 'json',
                    success: function(response) { if (response.sukses) { Swal.fire({icon: 'success', title: 'Berhasil!', text: response.sukses}); listjadwal(); }}
                });
            }
