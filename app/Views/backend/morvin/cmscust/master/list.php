        ...
        <tbody>
            ...
            ?>
                <tr>
                    ...
                    <td class="p-1">
                        &nbsp; <?= esc($data['nama_master']) ?> <a class="text-danger" title="<?= ($jdata != 0) ? '(' . $toltip . ')' : '' ?>"><?= ($jdata != 0) ? '(' . $jdata . ')' : '' ?></a>
                    </td>

                    <td class="text-center p-1">
                        <?php if ($ubah == 1) { ?>
                            <button type="button" onclick="toggle('<?= $id_masterdata ?>')" class="btn btn-circle btn-sm btn-light p-1" title="<?= $sts_master == 1 ? 'Non Aktifkan' : 'Aktifkan' ?>"><i class="<?= $sts_master == 1 ? 'fas fa-check-circle text-success' : 'far fa-eye text-danger' ?>"></i></button>
                            <button type="button" class="btn btn-light btn-sm p-1" onclick="edit('<?= $id_masterdata ?>')">
                                <i class="icon fas fa-edit text-info"></i>
                            </button>
                        <?php } else { ?>
                            <button type="button" class="btn btn-circle btn-sm btn-light p-1" title="<?= $sts_master == 1 ? 'Non Aktifkan' : 'Aktifkan' ?>"><i class="<?= $sts_master == 1 ? 'fas fa-check-circle text-success' : 'far fa-eye text-danger' ?>"></i></button>
