            ...
            foreach ($list as $data):
                ...
                <tr>
                    ...
                    <td class="text-center p-1">
                        <?php if ($ubah == 1) { ?>
                            <img class="img-circle elevation-2 pointer p-0" onclick="gantifoto('<?= $data['fasilitas_id'] ?>')"
                                src="<?= base_url('public/img/informasi/fasilitas/' . $data['cover_foto']) ?>"
                                title="Ganti Cover" width="60px">
                        <?php } else { ?>
                            <img class="img-circle elevation-2 p-0"
                                src="<?= base_url('public/img/informasi/fasilitas/' . $data['cover_foto']) ?>" width="60px">
                        <?php } ?>

                    </td>

                    <td class="p-1">
                        <a class="text-primary" href="<?= base_url('fasilitas/detail/' . $data['fasilitas_id']) ?>"
                            title="Detail fasilitas"> <?= esc($data['fasilitas']) ?></a>
                    </td>
                    ...
                    <td class="text-center p-1">
                        <?php if ($ubah == 1) { ?>
                            <?php if ($data['sts'] == '1') { ?>
                                <button type="button" onclick="toggledef('<?= $data['fasilitas_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['sts'] ? 'btn-light' : 'btn-success' ?>"
                                    title="<?= $data['sts'] ? 'Non aktifkan default halaman' : 'Jadikan Default Tampilan Awal' ?>"><i
                                        class="nav-icon fas fa-star text-success"></i>
                                </button>
                            <?php } else { ?>
                                <button type="button" onclick="toggledef('<?= $data['fasilitas_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['sts'] ? 'btn-light' : 'btn-light' ?>"
                                    title="<?= $data['sts'] ? 'Nonaktifkan' : 'Jadikan Default Tampilan Awal' ?>"><i
                                        class="nav-icon fas fa-star text-danger"></i>
                                </button>
                            <?php } ?>
                        <?php } else { ?>
                            <button type="button" class="btn btn-circle btn-sm p-1 "><i
                                    class="nav-icon fas fa-star text-secondary"></i>
                            </button>
                        <?php } ?>

                        <?php if ($ubah == 1) { ?>
                            <button type="button" title="Edit Data" class="btn btn-light btn-sm p-1"
                                onclick="edit('<?= $data['fasilitas_id'] ?>')">
                        ...
                        <?php if ($hapus == 1) { ?>
                            <button type="button" title="Hapus Data" class="btn btn-light btn-sm p-1"
                                onclick="hapus('<?= $data['fasilitas_id'] ?>','<?= $data['fasilitas'] ?>')">
