        ...
        <tbody>
            ...
            foreach ($list as $data):
                ...
                <tr>
                    ...
                    <td class="p-1"> <a class="text-primary"
                            href="<?= base_url('transparansi/detail/' . $data['transparan_id']) ?>"
                            title="Detail Transparansi"> <?= esc($data['judul']) ?>
                            <?php if ($data['vawal'] == '1') { ?>
                    ...
                    <td class="text-center p-1">
                        <?php if ($akses == 1) { ?>
                            <?php if ($data['sts'] == '1') { ?>
                                <i class="icon fas fa-check-circle text-success ml-0 text-center pointer"
                                    onclick="toggle(<?= $data['transparan_id'] ?>)" title="Klik disini untuk Non Aktifkan"></i>
                                <label class="pointer" title="Klik disini untuk Non Aktifkan"
                                    onclick="toggle(<?= $data['transparan_id'] ?> )"> Aktif </label>
                            <?php } else { ?>
                                <i class="icon fas fa-times-circle text-danger ml-0 text-center pointer"
                                    onclick="toggle(<?= $data['transparan_id'] ?>)" title="Klik disini untuk terbitkan"></i>
                                <label class="text-danger pointer" title="Klik disini untuk aktifkan"
                                    onclick="toggle(<?= $data['transparan_id'] ?>)"> Non Aktif </label>
                            <?php } ?>
                        <?php } else { ?>
                            <?php if ($data['sts'] == '1') { ?>
                    ...
                    <td class="text-center p-1">
                        <?php if ($ubah == 1) { ?>
                            <?php if ($data['vawal'] == '1') { ?>
                                <button type="button" onclick="toggledef('<?= $data['transparan_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['vawal'] ? 'btn-light' : 'btn-success' ?>"
                                    title="<?= $data['vawal'] ? 'Non aktifkan default halaman' : 'Jadikan Default Tampilan Awal' ?>"><i
                                        class="nav-icon fas fa-star text-success"></i>
                                </button>
                            <?php } else { ?>
                                <button type="button" onclick="toggledef('<?= $data['transparan_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['vawal'] ? 'btn-light' : 'btn-light' ?>"
                                    title="<?= $data['vawal'] ? 'Nonaktifkan' : 'Jadikan Default Tampilan Awal' ?>"><i
                                        class="nav-icon fas fa-star text-danger"></i>
                                </button>
                            <?php } ?>
                        <?php } else { ?>
                        ...
                        <?php if ($ubah == 1) { ?>
                            <button type="button" class="btn btn-light btn-sm p-1"
                                onclick="edit('<?= $data['transparan_id'] ?>')">
                        ...
                        <?php if ($hapus == 1) { ?>
                            <button type="button" class="btn btn-light btn-sm p-1"
                                onclick="hapus('<?= $data['transparan_id'] ?>','<?= $data['judul'] ?>')">
