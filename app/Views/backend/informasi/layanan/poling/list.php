<?php if ($list) { ?>
    ...
    <?php } ?>
    <small class="text-secondary"> Untuk <strong class="text-danger">Tutup Polling/Jajak Pendapat </strong> silahkan Non
        aktifkan pada pertanyaan..! </small>
    <hr>
                ...
                foreach ($list as $data):
                    ...
                    <tr>
                        ...
                        <td class="p-2">
                            ...
                            <?php if ($data['type'] == 'Jawaban') { ?>
                                ...
                                <div class="progress p-0" style="height: 20px;">
                                    <div class="progress-bar progress-bar-striped progress-bar p-0" role="progressbar"
                                        style="width: <?= $prosentase ?>%" aria-valuenow="10" aria-valuemin="0" aria-valuemax="100">
                                        <?= $prosentase ?>%
                                    </div>
                                </div>
                            <?php } ?>
                        ...
                        <td class="text-center p-2">
                            <?php if ($ubah == 1) { ?>
                                <button type="button" onclick="toggle('<?= $data['poling_id'] ?>')"
                                    class="btn btn-circle btn-sm btn-light"
                                    title="<?= $data['status'] == 'Y' ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                                        class="<?= $data['status'] == 'Y' ? 'fas fa-check-circle text-success' : 'far fa-eye text-danger' ?>"></i></button>
                            <?php } else { ?>
                                <button type="button" class="btn btn-circle btn-sm btn-light"
                                    title="<?= $data['status'] == 'Y' ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                                        class="<?= $data['status'] == 'Y' ? 'fas fa-check-circle text-success' : 'far fa-eye text-danger' ?>"></i></button>
                            <?php } ?>
                            <?php if ($ubah == 1) { ?>
...
<?php } else { ?>
    <center>
        ...
        <div class="alert alert-danger alert-dismissible fade show mb-0" role="alert">
            ...
            Maaf, data polling untuk layanan <b><?= esc($nama) ?></b> saat ini tidak tersedia, Silakan lakukan <i
                class="fas fa-copy text-warning"></i> Duplikasi Polling <br>pada menu <a
                href="<?= base_url('layanan/all') ?>" class="alert-link">Layanan</a> untuk mulai menggunakannya.
        </div>
    </center>
<?php } ?>
<script>
