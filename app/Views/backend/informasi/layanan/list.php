    ...
    <table id="listlayanan" class="table table-hover table-striped table-bordered">
        ...
        <tbody>
            ...
            foreach ($list as $data):
                ...
                <tr>
                    <td>
                        <input type="checkbox" name="informasi_id[]" class="centang_id"
                            value="<?= $data['informasi_id'] ?>">
                    </td>
                    <td><?= $nomor ?></td>
                    ...
                    <td>
                        <?php if ($tambah == 1) { ?>
                            <?php if (esc($data['fileunduh']) != '') { ?>
                                <i class="fas fa-file-alt text-success pointer" onclick="uploadfile('<?= $data['informasi_id'] ?>')"
                                    title="Ganti file"></i>
                            <?php } else { ?>
                                <i class="far fa-file-alt pointer" onclick="uploadfile('<?= $data['informasi_id'] ?>')"
                                    title="Tambahkan file"></i>
                            <?php } ?>
                        <?php } ?>
                        ...
                        <?php if ($hapus == 1) { ?>
                            <?php if (esc($data['fileunduh']) != '') { ?>
                                <i class="far fa-trash-alt text-warning pointer"
                                    onclick="hapusfile('<?= $data['informasi_id'] ?>','<?= esc($data['nama']) ?>')"
                                    title="Hapus file yang disematkan"></i>
                            <?php } ?>
                        <?php } ?>
                    </td>
                    ...
                    <td class="text-center ">
                        ...
                        <?php if (esc($data['gambar']) == 'default.png') { ?>
                            ...
                            <span class="badge badge-soft-warning font-size-12 pointer" <?= $eksekusi ?>
                                title="Tambahkan Cover">No Cover </span>
                        <?php } else { ?>
                            <img class="img-circle elevation-2 pointer" title="Ganti Gambar" <?= $eksekusi ?>
                                src="<?= base_url('public/img/informasi/layanan/' . esc($data['gambar'])) ?>" height="100%"
                                width="55px">
                        <?php } ?>
                    ...
                    <td class="text-center ">
                        <?php if ($ubah == 1) { ?>
                            <button type="button" title="Edit Data" class="btn btn-light btn-sm p-1"
                                onclick="edit('<?= $data['informasi_id'] ?>')">
                                <i class="fa fa-edit text-primary"></i>
                            </button>
                            <?php if ($data['utm'] == '1') { ?>
                                <button type="button" onclick="toggle('<?= $data['informasi_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['utm'] ? 'btn-light' : 'btn-success' ?>"
                                    title="<?= $data['utm'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                                        class="fas fa-check-circle text-success"></i>
                                </button>
                            <?php } else { ?>
                                <button type="button" onclick="toggle('<?= $data['informasi_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['utm'] ? 'btn-info' : 'btn-light' ?>"
                                    title="<?= $data['utm'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                            ...
                            <?php if ($cekpoling) { ?>
                                <button type="button"
                                    onclick="window.location.href='<?= base_url('layanan-poling/' . $data['informasi_id']) ?>'"
                                    class="btn btn-circle btn-sm p-1 btn-light pointer" title="View Data Poling"><i
                                        class="nav-icon fas fa-copy text-success"></i>
                                </button>
                            <?php } else { ?>
                                <button type="button"
                                    onclick="duplikasipoling('<?= $data['informasi_id'] ?>','<?= esc($data['nama']) ?>')"
                                    class="btn btn-circle btn-sm p-1" title="Duplikasi Poling"><i
                                        class="fas fa-copy text-secondary"></i>
                                </button>
                            <?php } ?>
                        <?php } else { ?>
                        ...
                        <?php if ($hapus == 1) { ?>
                            <button type="button" title="Hapus Data" class="btn btn-light btn-sm p-1"
                                onclick="hapus('<?= $data['informasi_id'] ?>','<?= esc($data['nama']) ?>')">
