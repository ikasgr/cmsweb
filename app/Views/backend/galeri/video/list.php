<div id="kembali">
    ...
    <small class="text-secondary"> Pada Link Youtube Contoh: <span
            class="text-warning">https://www.youtube.com/watch?v=</span><strong
            class="text-danger">X_fh-xVmto0</strong>. Ambil kode yang warna <strong
            class="text-danger">Merah</strong></small>
                ...
                foreach ($list as $value):
                    ...
                    <tr>
                        ...
                        <td class="text-center ">
                            <img class="img-circle elevation-2 pointer" title="<?= esc($value['ket_video']) ?>"
                                src="https://img.youtube.com/vi/<?php echo esc($value['video_link']) ?>/mqdefault.jpg"
                                width="100" alt="<?= $value['video_id'] ?>">
                        </td>
                        ...
                        <td class="text-center ">
                            <?php if ($ubah == 1) { ?>
                                <?php if ($value['sts_v'] == '1') { ?>
                                    <button type="button" onclick="toggle(<?= $value['video_id'] ?>)"
                                        class="btn btn-circle btn-sm p-1 <?= $value['sts_v'] ? 'btn-light' : 'btn-success' ?>"
                                        title="<?= $value['sts_v'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                                            class="fas fa-check-circle text-success"></i>
                                    </button>
                                <?php } else { ?>
                                    <button type="button" onclick="toggle(<?= $value['video_id'] ?>)"
                                        class="btn btn-circle btn-sm p-1 <?= $value['sts_v'] ? 'btn-info' : 'btn-light' ?>"
                                        title="<?= $value['sts_v'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                            ...
                            <?php if ($hapus == 1) { ?>
                                <button type="button" title="Hapus Data" class="btn btn-light btn-sm p-1"
                                    onclick="hapus('<?= $value['video_id'] ?>','<?= esc($value['judul']) ?>')">
