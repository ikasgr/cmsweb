            ...
            foreach ($list as $value):
                ...
                <tr>
                    <td class="p-1 text-center">
                        <input type="checkbox" name="kritiksaran_id[]" class="centangid"
                            value="<?= $value['kritiksaran_id'] ?>">
                    </td>
                        ...
                        <?php if ($ubah == 1) { ?>
                            <?php if ($value['status'] == '1') { ?>
                                ...
                                <?php } elseif ($value['status'] == '1') { ?>
                                    <a class="pointer" onclick="toggle('<?= $value['kritiksaran_id'] ?>')"
                                        class="btn btn-circle btn-sm <?= $value['status'] ? 'btn-success' : 'btn-success' ?>"
                                        title="<?= $value['status'] ? 'Tampilkan Pesan ini' : '' ?>"><i
                            ...
                            <?php } elseif ($value['status'] == '0') { ?>
                                ...
                                <a><i class="mdi mdi-message-settings-variant text-warning font-18 p-0"
                                        title="<?= $value['judul'] ?>"></i></a>
                                <b><?= esc($value['nama']) ?></b>

                            <?php } else { ?>

                                <a class="pointer" onclick="toggle('<?= $value['kritiksaran_id'] ?>')"
                                    class="btn btn-circle btn-sm <?= $value['status'] ? 'btn-danger' : 'btn-success' ?>"
                                    title="<?= $value['status'] ? 'Jangan Tampilkan' : 'Aktifkan' ?>"><i
                                        class="fas fa-eye-slash text-danger"></i>
                                </a>
                                <?= esc($value['nama']) ?>
                            <?php } ?>
                        ...
                        <?php } else { ?>
                            <?php if ($value['status'] == '1') { ?>
                                ...
                                <?php } elseif ($value['status'] == '1') { ?>
                                    <a class="pointer"
                                        class="btn btn-circle btn-sm <?= $value['status'] ? 'btn-success' : 'btn-success' ?>"
                                        title="<?= $value['status'] ? 'Tidak Tampil di Suara Anda' : 'Sembunyikan' ?>"><i
                            ...
                            <?php } else { ?>
                                ...
                                <a class="pointer"
                                    class="btn btn-circle btn-sm <?= $value['status'] ? 'btn-danger' : 'btn-success' ?>"
                                    title="<?= $value['status'] ? 'Telah Tampil di Suara Anda' : 'Aktifkan' ?>"><i
                                        class="fas fa-eye-slash text-secondary"></i>
                                </a>
                                <?= esc($value['nama']) ?>
                            <?php } ?>
                    ...
                    <td class="text-center p-1">
                        ...
                        <?php if ($ubah == 1) { ?>
                            <?php if ($value['status'] == '1') { ?>
                                <button type="button" onclick="toggle('<?= $value['kritiksaran_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 btn-light" title="Telah ditanggapi"><i
                                        class="fas fa-check-circle text-info font-14"></i>
                                </button>
                            <?php } else if ($value['status'] == '2') { ?>
                                    <button type="button" onclick="toggle('<?= $value['kritiksaran_id'] ?>')"
                                        class="btn btn-circle btn-sm p-1 btn-light" title="Telah ditanggapi dan ditampilkan"><i
                                            class="far fa-check-circle font-14 text-success"></i>
                                    </button>
                            <?php } else { ?>
                                    <button type="button" class="btn btn-circle btn-sm p-1 btn-light" title="Belum ditanggapi"><i
                                            class="fas fa-arrow-circle-right font-14 text-warning"></i>
                                    </button>
                            <?php } ?>

                        <?php } else { ?>
                            <?php if ($value['status'] == '1') { ?>
                                <button type="button" class="btn btn-circle btn-sm p-1 btn-light" title="Telah ditanggapi"><i
                                        class="fas fa-check-circle font-14 text-info"></i>
                                </button>
                            <?php } else if ($value['status'] == '2') { ?>
                                    <button type="button" class="btn btn-circle btn-sm p-1 btn-light"
                                        title="Telah ditanggapi dan ditampilkan"><i
                                            class="far fa-check-circle font-14 text-success"></i>
                                    </button>
                            <?php } else { ?>
                                    <button type="button" class="btn btn-circle btn-sm p-1 btn-light" title="Belum ditanggapi"><i
                        ...
                        <?php if ($ubah == 1) { ?>
                            <button type="button" class="btn btn-light btn-sm p-1"
                                onclick="edit('<?= esc($value['kritiksaran_id']) ?>')">
                        ...
                        <?php if ($hapus == 1) { ?>
                            <button type="button" class="btn btn-light btn-sm"
                                onclick="hapus('<?= $value['kritiksaran_id'] ?>','<?= esc($value['judul']) ?>')">
