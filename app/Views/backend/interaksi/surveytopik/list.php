...
<?php } ?>
<?php if ($hapus == 1) { ?>
    <small class="text-dark"> Untuk reset data survei, dan menghapus semua data responden pada topik yang telah diisi, klik
        tombol <b class="text-danger"><i class="fas fa-recycle font-13"></i> dibawah..! </small>
                ...
                <tr>
                    ...
                    <td class="">
                        <?php if ($jpertanyaan != 0) { ?>
                            <a href="<?= base_url('survey/cetak/' . $data['survey_id']) ?>" target="_blank" title="Cetak"><i
                                    class="fas fa-print text-dark font-14"></i> </a>
                        <?php } else { ?>
                            <a class="text-light"><i class="fas fa-print text-secondary font-14"></i> </a>
                        <?php } ?>
                        <a class="text-primary" title="Kelola pertanyaan"
                            href="<?= base_url('survey/pertanyaan/' . $data['survey_id']) ?>">
                            <?= esc($data['nama_survey']) ?>
                            <?php if ($jpertanyaan != 0) { ?>
                                <span class="badge badge-soft-success font-size-12 pointer" title="Jumlah pertanyaan"
                                    style="font-size:12px">(<?= $jpertanyaan ?>) </span>
                            <?php } else { ?>
                                <span class="badge badge-soft-danger font-size-12 pointer" title="Jumlah pertanyaan"
                                    style="font-size:12px">(<?= $jpertanyaan ?>) </span>
                            <?php } ?>
                        </a>
                    </td>
                    ...
                    <td class=" text-center">
                        ...
                        if ($skor != 0) {
                            ...
                            <?php if ($dhasil >= $r1_stb && $dhasil <= $r2_stb) { ?>
                                <span class="badge badge-soft-danger font-size-12" title="Penilaian Akhir"
                                    style="font-size:12px"><?= $data['ket_stb'] ?> (<?= $dhasil ?>)</span>
                            <?php } elseif ($dhasil >= $r1_kb && $dhasil <= $r2_kb) { ?>
                                <span class="badge badge-soft-warning font-size-12" title="Penilaian Akhir"
                                    style="font-size:12px"><?= $data['ket_kb'] ?> (<?= $dhasil ?>)</span>

                            <?php } elseif ($dhasil >= $r1_b && $dhasil <= $r2_b) { ?>
                                <span class="badge badge-soft-info font-size-12" title="Penilaian Akhir" style="font-size:12px">
                                    <?= $data['ket_b'] ?> (<?= $dhasil ?>)</span>

                            <?php } elseif ($dhasil >= $r1_sb) { ?>
                                <span class="badge badge-soft-success font-size-12" title="Penilaian Akhir"
                                    style="font-size:12px"><?= $data['ket_sb'] ?> (<?= $dhasil ?>)</span>
                            <?php } ?>
                    ...
                    <td class=" text-center">
                        <?php if ($skor != 0) { ?>
                            <a href="<?= base_url('survey/pesan/' . $data['survey_id']) ?>">
                                <span class="badge badge-soft-primary font-size-12 pointer" title="Lihat Responden"
                                    style="font-size:12px">(<?= $jrespon ?>) </span>
                            </a>
                        <?php } else { ?>
                            <span class="badge badge-soft-secondary font-size-12 pointer" title="Belum ada Responden"
                                style="font-size:12px">(<?= $jrespon ?>) </span>
                        <?php } ?>
                    ...
                    <td class="text-center ">
                        ...
                        <?php if ($ubah == 1) { ?>
                            <button type="button" onclick="toggle('<?= $data['survey_id'] ?>','2')"
                                class="btn btn-circle btn-sm btn-light p-1"
                                title="<?= $data['lockisi'] == 1 ? 'Tidak Terkunci (Pengisian tidak dibatasi)' : 'Terkunci (Pengisian dibatasi)' ?>"><i
                                    class="<?= $data['lockisi'] == 1 ? 'fas fa-lock-open text-danger' : 'fas fa-lock text-success' ?>"></i></button>
                            <?php if ($jpertanyaan != 0) { ?>
                                <button type="button" onclick="toggle('<?= $data['survey_id'] ?>','1')"
                                    class="btn btn-circle btn-sm btn-light p-1"
                                    title="<?= $data['status'] == 1 ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                                        class="<?= $data['status'] == 1 ? 'fas fa-check-circle text-success' : 'far fa-eye text-danger' ?>"></i></button>
                            <?php } else { ?>
                                <button type="button" class="btn btn-light btn-sm p-1"><i
                                        class="far fa-eye text-secondary"></i></button>
                            <?php } ?>
                        <?php } else { ?>
                            <button type="button" class="btn btn-circle btn-sm btn-light p-1"
                                title="<?= $data['lockisi'] == 1 ? 'Tidak Terkunci (Pengisian tidak dibatasi)' : 'Terkunci (Pengisian dibatasi)' ?>"><i
                                    class="<?= $data['lockisi'] == 1 ? 'fas fa-lock-open text-danger' : 'fas fa-lock text-success' ?>"></i></button>
                            <button type="button" class="btn btn-light btn-sm p-1"><i
                                    class="fas fa-recycle text-secondary"></i></button>
                        <?php } ?>
                        <?php if ($ubah == 1) { ?>
                            <!-- edit -->
                        ...
                        <?php if ($skor != 0 && $jrespon != 0 && $hapus == 1) { ?>
                            <button type="button" class="btn btn-light btn-sm p-1"
                                onclick="resetsurvei('<?= $data['survey_id'] ?>','<?= $data['nama_survey'] ?>')"
                                title="Reset survei">
                        ...
                        <?php if ($skor == 0 && $jrespon == 0 && $jpertanyaan == 0 && $hapus == 1) { ?>
                            <button type="button" class="btn btn-light btn-sm p-1"
                                onclick="hapus('<?= $data['survey_id'] ?>','<?= $data['nama_survey'] ?>')">
