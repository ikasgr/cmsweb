<a href="<?= base_url('modul/') ?>" class="btn btn-warning btn-sm "><i class="far fa-arrow-alt-circle-left font-14"></i>
    Kembali</a>
...
<?php } ?>
<small class="text-secondary"> Modul Publik Adalah modul yang akan diakses publik, dan menjadi pilihan dalam pembuatan
    <strong class="text-danger">Menu, Sub Menu, Banner dll</strong> </small>
<hr>
                    ...
                    <td class="text-center p-2">
                        <?php if ($akses == 1) { ?>
                            <?php if ($data['stsmod'] == '1') { ?>
                                <button type="button" onclick="toggle('<?= $data['id_modpublic'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['stsmod'] ? 'btn-light' : 'btn-success' ?>"
                                    title="<?= $data['stsmod'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                                        class="fas fa-check-circle text-success"></i>
                                </button>
                            <?php } else { ?>
                                <button type="button" onclick="toggle('<?= $data['id_modpublic'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['stsmod'] ? 'btn-info' : 'btn-light' ?>"
                                    title="<?= $data['stsmod'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                                        class="nav-icon far fa-eye text-danger"></i>
                                </button>
                            <?php } ?>
                            <button type="button" class="btn btn-light btn-sm p-1"
                                onclick="edit('<?= $data['id_modpublic'] ?>')">
                                <i class="icon fas fa-edit text-warning"></i>
                            </button>
                            <button type="button" class="btn btn-light btn-sm p-1"
                                onclick="hapus('<?= $data['id_modpublic'] ?>')">
