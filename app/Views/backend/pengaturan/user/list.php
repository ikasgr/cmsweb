...
<?php if ($akses == 1) { ?>
    <button type="button" class="btn btn-social btn-flat btn-warning btn-sm"
        onclick="location.href=('<?= base_url('log-sesi'); ?>')"><i class="fas fa-glasses"></i> Log Sesi</button>
            ...
            <tbody>
                ...
                foreach ($list as $value):
                    ...
                    <tr>
                        ...
                        <td><img class="img-circle elevation-2 rounded <?= $ubah == 1 ? 'pointer' : '' ?>"
                                onclick="<?= $ubah == 1 ? "gantifoto('{$value['id']}')" : '' ?>"
                                src="<?= base_url('public/img/user/' . $profil) ?>" width="40px"></td>
                        ...
                        <td><?= esc($value['fullname']) ?>
                            <?= $viewopd ? "<br><a class='text-primary' title='Unit Kerja : " . esc($viewopd['nama_opd']) . "'>" . esc($viewopd['singkatan_opd']) . "</a>" : '' ?>
                        </td>
                        <td>
                            <?= $value['email'] ?><br>
                            <a class="badge badge-soft-<?= $value['login_attempts'] <= 2 ? 'info' : 'danger' ?> font-size-12 badge-pill pointer"
                                <?= $ubah == 1 && $value['login_attempts'] >= 3 ? 'onclick="toggle(\'' . $value['id'] . '\', \'' . $jtogel2 . '\')"' : '' ?> title="Upaya Login">
                        ...
                        <td class="text-center">
                            ...
                            <?php if ($ubah == 1): ?>
                                <button type="button" onclick="toggle('<?= $value['id'] ?>', '<?= $jtogel1 ?>')"
                                    class="mt-2 btn btn-circle btn-sm <?= $btnToggleClass ?>" title="<?= $toggleTitle ?>">
                                    <i class="nav-icon <?= $toggleIcon ?>"></i>
                                </button>
                            <?php endif; ?>

                            <!-- View Statistics -->
                            <button type="button" class="mt-2 btn btn-light btn-sm p-1" title="Lihat Statistik Postingan"
                                onclick="lihat('<?= $value['id'] ?>', '<?= $jenisgrp ?>')">
                                <i class="nav-icon fa fa-search text-info"></i>
                            </button>

                            <!-- Edit -->
                            <?php if ($ubah == 1): ?>
                                <button type="button" class="mt-2 btn btn-light btn-sm p-1"
                                    onclick="edit('<?= $value['id'] ?>', '<?= $jenisgrp ?>')">
                            ...
                            <?php if ($jenisgrp != '1' && $hapus == 1): ?>
                                <button type="button" class="mt-2 btn btn-light btn-sm p-1"
                                    onclick="hapus('<?= $value['id'] ?>', '<?= esc($value['fullname']) ?>')">
