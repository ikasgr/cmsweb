<a href="<?= base_url('menu/') ?>" class="btn btn-warning btn-sm "><i class="far fa-arrow-alt-circle-left font-14"></i>
    Kembali</a>
<?php if ($tambah == 1) { ?>
                ...
                <tr>
                    ...
                    <td class="p-1">
                        <?php if (esc($data['iconsm']) != '') { ?>
                            &nbsp; <i class="<?= esc($data['iconsm']) ?> text-secondary"
                                title="<?= esc($data['iconsm']) ?>"></i><a class="text-warning font-12"></a>
                        <?php } ?>
                        <?php if ($data['parentsm'] == 'Y') { ?>
                            <a class="text-primary" href="<?= base_url('subsubmenu/' . $data['submenu_id']) ?>"
                                title="Kelola Sub Sub Menu">&nbsp;<?= esc($data['nama_submenu']) ?></a> <i
                    ...
                    <td class="p-1">
                        ...
                        <a target="_blank"
                            href="<?php echo base_url(esc($data['link_submenu'])) ?>"><?= esc($data['link_submenu']) ?> </a>
                    ...
                    <td class="text-center p-1">
                        <?php if ($ubah == 1) { ?>
                            <?php if ($data['stssubmenu'] == '1') { ?>
                                <button type="button" onclick="toggle('<?= $data['submenu_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['stssubmenu'] ? 'btn-light' : 'btn-success' ?>"
                                    title="<?= $data['stssubmenu'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                                        class="fas fa-check-circle text-success"></i>
                                </button>
                            <?php } else { ?>
                                <button type="button" onclick="toggle('<?= $data['submenu_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['stssubmenu'] ? 'btn-light' : 'btn-light' ?>"
                                    title="<?= $data['stssubmenu'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
