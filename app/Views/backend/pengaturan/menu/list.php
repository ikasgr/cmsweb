    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="width: 180px;">
                <button type="button" class="btn btn-outline-secondary waves-effect waves-light mr-4 tambahmenusecond">
                    <i class="fas fa fa-plus-circle"></i>Tambah Secondary
                </button>
            </td>
                ...
                <tr>
                    ...
                    <td class="p-1">
                        ...
                        <?php if ($data['parent'] == 'Y') { ?>
                            <a class="text-primary" href="<?= base_url('submenu/' . $data['menu_id']) ?>"
                                title="Kelola Sub Menu"> <?= esc($data['nama_menu']) ?></a> <i class="mdi mdi-chevron-down"></i>
                    ...
                    <?php } else { ?>
                        <td class="p-1">
                            ...
                            <a target="_blank" href="<?php echo base_url($data['menu_link']) ?>"><?= esc($data['menu_link']) ?>
                            </a>
                        </td>
                    <?php } ?>
                    ...
                    <td class="p-1 text-center">
                        <?php if ($data['icon'] != '') { ?>
                            <a class="text-warning font-13"> <i class="<?= $data['icon'] ?> text-secondary"
                                    title="<?= $data['icon'] ?>"></i></a>
                        <?php } else { ?>
                            -
                        <?php } ?>
                    ...
                    <td class="text-center p-1">
                        <?php if ($ubah == 1) { ?>
                            <?php if ($data['stsmenu'] == '1') { ?>
                                <button type="button" onclick="toggle('<?= $data['menu_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['stsmenu'] ? 'btn-light' : 'btn-success' ?>"
                                    title="<?= $data['stsmenu'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
                                        class="fas fa-check-circle text-success"></i>
                                </button>
                            <?php } else { ?>
                                <button type="button" onclick="toggle('<?= $data['menu_id'] ?>')"
                                    class="btn btn-circle btn-sm p-1 <?= $data['stsmenu'] ? 'btn-info' : 'btn-light' ?>"
                                    title="<?= $data['stsmenu'] ? 'Non Aktifkan' : 'Aktifkan' ?>"><i
