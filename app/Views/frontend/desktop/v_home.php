...
<?= $this->section('v_menu') ?>
<?= $this->include('frontend/desktop/v_menu') ?>
<?= $this->endSection() ?>
                    ...
                    <div class="carousel-item <?= $active ?> hero-section">
                        ...
                        <div class="hero-content">
                            <h1 class="hero-title animate__animated animate__fadeInDown">
                                <?= esc($konfigurasi->nama ?? 'Selamat Datang') ?>
                            </h1>
        ...
        <div class="row align-items-center">
            <div class="col-lg-6 mb-4 mb-lg-0">
                ...
                <h2 class="font-weight-bold mb-4" style="font-family: 'Montserrat'; color: #101820; font-size: 2.5rem;">
                    Selamat Datang di <br><span style="color: #D4AF37;"><?= esc($konfigurasi->nama) ?></span>
                </h2>
            ...
            <div class="col-lg-6">
                ...
                <div class="position-relative p-3" style="border: 2px solid #D4AF37; border-radius: 8px;">
                    <img src="<?= base_url('public/img/banner/' . ($banner[0]['banner_image'] ?? 'banner1.png')) ?>"
                        ...
                        alt="Gereja">
                    <?php if (isset($konfigurasi->banner_video) && !empty($konfigurasi->banner_video)): ?>
                        ...
                        <div id="newsCarousel" class="carousel slide" data-ride="carousel">
                            <div class="carousel-inner">
                                ...
                                foreach ($beritautama as $berita):
                                    $n++;
                                    $active = ($n == 1) ? 'active' : '';
                                    $img_berita = image_url('informasi/berita/' . esc($berita['gambar']));
                                    ?>
                        ...
                        <div class="card church-card h-100 p-0">
                            <div class="overflow-hidden">
                                <a href="<?= base_url($data['slug_berita']) ?>">
                                    <img src="<?= image_url('informasi/berita/' . esc($data['gambar'])) ?>" class="card-img-top"
...
<!-- PRODUK UMKM SECTION -->
<!-- ================================================== -->
<?php if (!empty($produk_featured)): ?>
    <section class="section-padding" style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%);">
        <div class="container">
            <div class="section-header-church">
                <h2><i class="fas fa-store text-warning"></i> Produk UMKM</h2>
                <div class="separator"></div>
                <p class="text-muted mt-3">Dukung produk-produk lokal berkualitas dari UMKM kami</p>
            </div>

            <div class="row">
                <?php foreach ($produk_featured as $produk):
                    $gambar_produk = !empty($produk['gambar'])
                        ? base_url('/public/img/produk/' . esc($produk['gambar']))
                        : base_url('/public/img/no_image.png');
                    $harga_display = !empty($produk['harga_promo']) && $produk['harga_promo'] > 0
                        ? $produk['harga_promo']
                        : $produk['harga'];
                    $wa_link = 'https://wa.me/' . preg_replace('/^0/', '62', $produk['whatsapp_admin'] ?? '')
                        . '?text=' . urlencode('Halo, saya tertarik dengan produk: ' . $produk['nama_produk']);
                    ?>
                    <div class="col-lg-3 col-md-4 col-6 mb-4">
                        <div class="card h-100 border-0 shadow-sm product-card"
                            ...
                            <div class="position-relative" style="height: 180px; overflow: hidden;">
                                <img src="<?= $gambar_produk ?>" class="w-100 h-100"
                                    style="object-fit: cover; transition: transform 0.3s ease;"
                                    alt="<?= esc($produk['nama_produk']) ?>">

                                <?php if (!empty($produk['harga_promo']) && $produk['harga_promo'] > 0): ?>
                                ...
                                <?php if ($produk['featured'] == '1'): ?>
                                    <span class="badge bg-warning text-dark position-absolute"
                                        style="top: 10px; left: 10px; font-size: 11px;">
                            ...
                            <div class="card-body p-3">
                                <small
                                    class="text-warning fw-bold text-uppercase"><?= esc($produk['nama_kategori'] ?? 'Produk') ?></small>
                                <h6 class="mt-2 mb-2 fw-bold" style="font-size: 0.95rem; line-height: 1.4; min-height: 42px;">
                                    <a href="<?= base_url('produk/' . esc($produk['slug_produk'])) ?>"
                                        class="text-dark text-decoration-none">
                                        <?= esc(character_limiter($produk['nama_produk'], 40)) ?>
                                    </a>
                                </h6>
                                ...
                                <div class="mb-2">
                                    <?php if (!empty($produk['harga_promo']) && $produk['harga_promo'] > 0): ?>
                                        <small class="text-decoration-line-through text-muted">Rp
                                            <?= number_format($produk['harga'], 0, ',', '.') ?></small><br>
                                        <span class="text-danger fw-bold" style="font-size: 1.1rem;">Rp
                                            <?= number_format($produk['harga_promo'], 0, ',', '.') ?></span>
                                    <?php else: ?>
                                        <span class="text-primary fw-bold" style="font-size: 1.1rem;">Rp
            ...
            <div class="text-center mt-4">
                <a href="<?= base_url('produk') ?>" class="btn btn-warning text-dark px-5 py-2"
...
<!-- ================================================== -->
<?php if (!empty($majelis) && is_array($majelis) && count($majelis) > 0): ?>
    <section class="section-padding bg-light">
        <div class="container">
            <div class="section-header-church">
                <h2><i class="fas fa-users text-warning"></i> Majelis Jemaat</h2>
                <div class="separator"></div>
                <p class="text-muted mt-3">Pelayan Tuhan yang melayani jemaat dengan setia</p>
            </div>

            <div class="owl-carousel owl-theme" id="majelisSlider">
                <?php foreach ($majelis as $data):
                    $img_majelis = !empty($data['gambar']) 
                        ? base_url('/public/img/informasi/majelis/' . esc($data['gambar'])) 
                        : base_url('/public/img/user/default.png');
                    $jabatan = !empty($data['nama_jabatan']) ? $data['nama_jabatan'] : ($data['jenis_jabatan'] ?? 'Majelis');
                    ?>
                    <div class="item p-3">
                        <div class="bg-white rounded shadow-sm p-4 text-center h-100" style="border-bottom: 3px solid #D4AF37;">
                            <img src="<?= $img_majelis ?>" class="rounded-circle mx-auto mb-3 shadow"
                                style="width: 100px; height: 100px; object-fit: cover; border: 3px solid #D4AF37;" 
                                alt="<?= esc($data['nama'] ?? 'Majelis') ?>">
                            <h6 class="font-weight-bold text-dark mb-1"><?= esc($data['nama'] ?? '-') ?></h6>
                            <small class="text-warning text-uppercase fw-bold"><?= esc($jabatan) ?></small>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
            ...
            <!-- View All Button -->
            <div class="text-center mt-4">
                <a href="<?= base_url('majelis') ?>" class="btn btn-outline-warning text-dark px-4 py-2" style="border-radius: 30px;">
                    <i class="fas fa-users"></i> Lihat Semua Majelis
                </a>
            </div>
        </div>
    </section>
