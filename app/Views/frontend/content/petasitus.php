                    ...
                    <div class=" container-grid projects-wrapper">
                            ...
                            <div class="list-group mt-1">
                                ...
                                if ($utama['parent'] == 'N') {
                                    if ($utama['linkexternal'] == 'N') { ?>
                                        <li class="list-group-item list-group-item-action p-2"><a target="<?= $utama['target'] ?>"
                                                href="<?= base_url(esc($utama['menu_link'])) ?>">
                                                <i class="<?= esc($utama['icon']) ?>"></i> <?= esc($utama['nama_menu']) ?> </a></li>
                                    <?php } else { ?>
                                        <li class="list-group-item list-group-item-action p-2"><a target="<?= $utama['target'] ?>"
                                                href="<?= esc($utama['menu_link']) ?>"> <i class="<?= esc($utama['icon']) ?>"></i>
                                ...
                                if ($utama['parent'] == 'Y') { ?>
                                    <li class="list-group-item list-group-item-action p-1">
                                        <a><i class="<?= esc($utama['icon']) ?>"></i><?= esc($utama['nama_menu']) ?> <i
                                                class="fas fa-caret-down"></i></a>
                                        <ul>
                                            <?php foreach ($set as $submenu) {
                                                if ($submenu['linkexternalsm'] == 'N') { ?>
                                                    <li class="list-group-item list-group-item-action p-2"><a
                                                            target="<?= $submenu['targetsm'] ?>"
                                                            href="<?= base_url(esc($submenu['link_submenu'])) ?>"><i
                                                                class="<?= esc($submenu['iconsm']) ?>"></i>
                                                            <?= esc($submenu['nama_submenu']) ?> </a></li>
                                                <?php } else { ?>
                                                    <li class="list-group-item list-group-item-action p-2"><a
                                                            target="<?= $submenu['targetsm'] ?>"
                                                            href="<?= esc($submenu['link_submenu']) ?>"><i
                                                                class="<?= esc($submenu['iconsm']) ?>"></i>
                                                            <?= esc($submenu['nama_submenu']) ?> </a></li>
                                                <?php } ?>
                                            <?php } ?>
                        ...
                        <?php foreach ($setfot as $fm) {
                            if ($fm['linkexternal'] == 'N') { ?>
                                <div class="list-group mt-1">
                                    <li class="list-group-item list-group-item-action p-2"><a target="<?= $fm['target'] ?>"
                                            href="<?= base_url(esc($fm['menu_link'])) ?>"><i
                                                class="<?= esc($fm['icon']) ?>"></i> <?= esc($fm['nama_menu']) ?> <small
                                                class="text-warning"> (Bottom Menu) </small> </a></li>

                                <?php } else { ?>
                                    <li class="list-group-item list-group-item-action p-2"><a target="<?= $fm['target'] ?>"
                                            href="<?= esc($fm['menu_link']) ?>"><i class="<?= esc($fm['icon']) ?>"></i>
                                            <?= esc($fm['nama_menu']) ?> <small class="text-warning"> (Bottom Menu) </small>
                                        </a></li>
                                <?php } ?>
