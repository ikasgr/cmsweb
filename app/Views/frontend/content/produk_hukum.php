            ...
            <div class="col-md-12" data-aos="fade-right" id="content">
                ...
                <?php if ($produkhukum) { ?>
                    ...
                    <div class="alert alert-info" style='background-color:#f4f4f4; border-color:#e3e3e3;'>
                        Informasi mengenai peraturan, keputusan, dan/atau kebijakan yang mengikat dan/atau berdampak bagi
                        publik dapat diunduh pada list dibawah.
                        Jika data yang dicari tidak ditemukan, Silahkan klik <b class="pointer"
                            onclick="window.location.href='<?= base_url('masukansaran') ?>'">disini</b>, untuk lakukan
                            ...
                            <div class="card p-1">
                                <div class="card-header p-3" id="heading<?= $data['produk_id'] ?>">
                                    <h6 class="m-0 font-14">
                                        <a href="#collapse<?= $data['produk_id'] ?>" class="text-dark" data-toggle="collapse"
                                            aria-expanded="true" aria-controls="collapse<?= $data['produk_id'] ?>">
                                            <i class="fa fa-balance-scale"></i> <?= strtoupper(esc($data['nama_produk'])) ?>
                                        </a>
                                    </h6>
                                </div>

                                <div id="collapse<?= $data['produk_id'] ?>" class="collapse showx"
                                    aria-labelledby="heading<?= $data['produk_id'] ?>" data-parent="#accordion">
                                        ...
                                        foreach ($set as $datasub) {
                                            ...
                                            <ul style="list-style:none;">
                                                <?php if (esc($datasub['file_kathukum']) != '-' && (esc($datasub['file_kathukum']) != null)) { ?>
                                                    <li class="pb-0">
                                                        <a href="<?= base_url('public/unduh/produkhukum/' . esc($datasub['file_kathukum'])) ?>"
                                                            title="Download file" target="_blank"><i
                                                                class="fas fa-file-alt text-primary pointer font-16"></i>
                                                            <?= (esc($datasub['nama_kathukum'])) ?></a>
                                                    </li>
                                                <?php } else { ?>
                                                ...
                                                foreach ($set2 as $datasubsub) {
                                                    ...
                                                    ?>
                                                    <li class="">
                                                        <a href="<?= base_url('public/unduh/produkhukum/' . esc($datasubsub['file_subkathukum'])) ?>"
                                                            title="Download file" target="_blank"> <?= $no ?>.
                                                            <?= esc($datasubsub['nama_subkathukum']) ?></a>
                                                    </li>
                    ...
                    <?php } else { ?>
                        <div class="alert alert-danger text-center" style='background-color:#FAEBD7; border-color:#e3e3e3;'>
                            <a style='color:red'>Belum Ada data Produk Hukum.!</a><br> Punya pertanyaan, keluhan, masukan
                            atau saran, silahkan klik <b class="pointer"
