                        ...
                        <?php foreach ($ebook as $data) {
                            ...
                            <div class="col-lg-4 col-sm-6 photo-gallery mb-4">
                                <div class="card h-100">
                                    <a href="<?= base_url('bacabuku/' . esc($data['fileebook'])) ?>" title="Baca Buku"
                                        target="_blank" onclick="updatehit('<?= $data['ebook_id'] ?>')"><img
                                            class="card-img-top"
                                            src="<?= base_url('/public/img/ebook/thumb/' . 'thumb_' . esc($data['gambar'])) ?>"
                                            alt=""></a>
                                    ...
                                    <h5 class="card-title">
                                        <a href="<?= base_url('bacabuku/' . esc($data['fileebook'])) ?>" title="Baca Buku"
                                            target="_blank" onclick="updatehit('<?= $data['ebook_id'] ?>')">
                                            <?= esc($data['judul']) ?> </a>
                                    </h5>
                                    <div class="metadata">
                                        <span class="date small"><i class="fa fa-calendar"></i>
                                            <?= date_indo($data['tanggal']) ?></span>
                                        | <span class="date pointer"
                                            onclick="lihatbook('<?= $data['ebook_id'] ?>','<?= esc($data['kategoriebook_nama']) ?>')"><i
                                                class="fa fa-search"></i>View Detail</span>
                                    </div>
                                    <!-- </div> -->
                                </div>
                            </div>
