-- ============================================
-- Tabel Pesanan UMKM (SAFE VERSION - No External FK)
-- Date: 2025-10-08
...
-- ============================================
-- Add Foreign Keys (Internal Only)
-- ============================================

-- FK untuk detail ke pesanan
ALTER TABLE `custome__pesanan_detail`
  ADD CONSTRAINT `fk_detail_pesanan` FOREIGN KEY (`pesanan_id`) 
  REFERENCES `custome__pesanan` (`pesanan_id`) ON DELETE CASCADE;

-- FK untuk tracking ke pesanan
ALTER TABLE `custome__pesanan_tracking`
  ADD CONSTRAINT `fk_tracking_pesanan` FOREIGN KEY (`pesanan_id`) 
  REFERENCES `custome__pesanan` (`pesanan_id`) ON DELETE CASCADE;
...
-- 7. Tidak ada FK ke tabel produk untuk fleksibilitas
-- ============================================

-- ============================================
-- Test Query
-- ============================================
-- SELECT * FROM custome__pesanan ORDER BY pesanan_id DESC LIMIT 10;
-- SELECT * FROM custome__pesanan_detail WHERE pesanan_id = 1;
-- SELECT * FROM custome__pesanan_tracking WHERE pesanan_id = 1 ORDER BY tgl_update DESC;
